...
public class DragSortController extends SimpleFloatViewManager implements View.OnTouchListener, GestureDetector.OnGestureListener {
    ...
    private int mDragInitMode = ON_DOWN; //改造成支持多种Mode (如ON_DRAG|ON_LONG_PRESS)
...
//    }
    ...
    public int viewIdHitPosition(MotionEvent ev, int id) {
        ...
        if (touchPos != AdapterView.INVALID_POSITION && touchPos >= numHeaders
            ...
            if (dragBox != null && dragBox.getVisibility() != View.GONE) {
    ...
    public boolean onDown(MotionEvent ev) {
        ...
        if (mHitPos != MISS && (mDragInitMode & ON_DOWN) == ON_DOWN) {
    ...
    public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
        ...
        if (mCanDrag && !mDragging && (mHitPos != MISS || mFlingHitPos != MISS)) {
            if (mHitPos != MISS) {
                if ((mDragInitMode & ON_DRAG) == ON_DRAG && Math.abs(y2 - y1) > mTouchSlop && mSortEnabled) {
                    startDrag(mHitPos, deltaX, deltaY);
                }
                else if ((mDragInitMode & ON_DOWN) != ON_DOWN && Math.abs(x2 - x1) > mTouchSlop && mRemoveEnabled)
                {
                    mIsRemoving = true;
    ...
    public void onLongPress(MotionEvent e) {
        ...
        if (mHitPos != MISS && (mDragInitMode & ON_LONG_PRESS) == ON_LONG_PRESS) {
